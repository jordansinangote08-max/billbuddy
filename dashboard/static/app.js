...
function dueLabel(value) {
  ...
  if (days === null) {
    return { text: "Due date unavailable", soon: false };
  }
 ...
 * Bank logo mapping.
 *
 * These paths expect the files to be inside:
 * dashboard/static/logos/
 *
 * Supported filenames:
 * unionbank.png
 * rcbc.png
 * eastwest.png
 *
 * If a logo does not exist, BillBuddy falls back to initials.
 */
function bankLogo(bank) {
  ...
  const logos = {
    unionbank: "/static/logos/unionbank.png",
    rcbc: "/static/logos/rcbc.png",
    eastwest: "/static/logos/eastwest.png"
  };
...
function bankLogoHtml(bank) {
  ...
  const initials = escapeHtml(
    String(bank || "?").slice(0, 2).toUpperCase()
  );
  ...
  return `
    <img
      src="${logo}"
      alt="${escapeHtml(bank)} logo"
      onerror="
        this.style.display='none';
        this.parentElement.querySelector('.logo-fallback').style.display='block';
      "
    >
          ...
          <td>
            <div class="table-bank">
              <div class="table-logo">
                ${bankLogoHtml(bill.bank_lender || "Unknown")}
              </div>
              <span>${escapeHtml(bill.bank_lender)}</span>
            </div>
          </td>
